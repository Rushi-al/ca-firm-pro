...
const EMPTY_UPDATE = {
  status:'in_progress', isItrFiled: false, isAccountsCompleted: false, filedDate:'',
  notes:'',
};
...
export default function IncomeTaxPage() {
  ...
  const openEdit = rec => {
    setUpd({
      status:          rec.status,
      isItrFiled:      rec.isItrFiled || false,
      isAccountsCompleted: rec.isAccountsCompleted || false,
      ...
      <div className="bg-[#0f172a] border border-slate-800 rounded-xl overflow-hidden overflow-x-auto">
        <table className="w-full text-sm min-w-[760px]">
          <thead><tr>
            ...
            <th className="th">Due Date</th><th className="th">A/C Completed</th>
            <th className="th">ITR Filed</th>
              ...
              : records.map(r => {
                ...
                return (
                  <tr key={r._id} className={`hover:bg-slate-800/30 transition-colors ${od?'bg-red-500/[0.03]':''}`}>
                    ...
                    <td className="td">
                      {r.isAccountsCompleted ? <span className="text-xs text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded border border-emerald-400/20">Yes</span> : <span className="text-xs text-slate-400 bg-slate-800 px-2 py-0.5 rounded border border-slate-700">No</span>}
                    </td>
                    <td className="td">
                      {r.isItrFiled ? <span className="text-xs text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded border border-emerald-400/20">Yes</span> : <span className="text-xs text-slate-400 bg-slate-800 px-2 py-0.5 rounded border border-slate-700">No</span>}
                    </td>
                    <td className="td">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <div className="flex gap-4 items-end pb-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-slate-700 bg-slate-800 text-amber-500 focus:ring-amber-500 focus:ring-offset-slate-900" 
                  checked={upd.isAccountsCompleted} onChange={e => setUpd(u => ({ ...u, isAccountsCompleted: e.target.checked }))} />
                <span className="text-sm text-slate-300">A/C Completed</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-slate-700 bg-slate-800 text-amber-500 focus:ring-amber-500 focus:ring-offset-slate-900" 
                  checked={upd.isItrFiled} onChange={e => setUpd(u => ({ ...u, isItrFiled: e.target.checked }))} />
                <span className="text-sm text-slate-300">ITR Filed</span>
              </label>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 mb-3">
