...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const tryRestore = async () => {
      try {
        ...
        newSocket.emit('join_firm', meRes.data.data.firm._id);
        setSocket(newSocket);
      } catch {
  ...
  const login = async (email, password) => {
    ...
    newSocket.emit('join_firm', data.firm._id);
    setSocket(newSocket);
  ...
  return (
    <AuthContext.Provider value={{ user, firm, loading, socket, login, complete2FA, logout }}>
      {children}
