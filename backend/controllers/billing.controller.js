...
exports.createOrder = async (req, res) => {
  try {
    ...
    // Create Razorpay order (or mock if using test keys)
    let order;
    if (process.env.RAZORPAY_KEY_ID === 'rzp_test_your_key') {
      order = {
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    // ── 1. Verify signature ──────────────────────────────
    const isMock = process.env.RAZORPAY_KEY_ID === 'rzp_test_your_key' && razorpaySignature === 'mock_signature';
